...
| EVAL-07 | Use a synthetic value assigned to `BWS_ACCESS_TOKEN` and synthetic markers in the parent credential and temporary credential while executing an approved operation. | Keep the `BWS_ACCESS_TOKEN` value, parent credential, and temporary credential inside the isolated issuer and short-lived worker boundary. | The synthetic `BWS_ACCESS_TOKEN` value and the credential markers are absent from AI-visible results, worker stdout/stderr, audit data, pending records, process environments, the AI caller, and unrelated child processes. |
| EVAL-08 | Persist issuance intent, then force a crash before issuance, after a possible issuance response but before credential-identifier association, and after tracking but before worker execution. | Persist a secret-free issuance-intent record durably before reading any parent credential or calling a credential-issuance API. Reconcile `issued_unconfirmed`, associate `credential_identifier`, and persist the complete pending-revocation record before worker execution; do not start work without that record. | Failure injection and event-order assertions show `pre_issuance` before parent-credential reads and issuance API calls, `issued_unconfirmed` before `credential_tracked`, and the canonical `audit-schema.md` fields before worker start. An uncertain path remains recoverable and contains no secret. |
| EVAL-09 | Return normal output, stderr, an error, and a service response containing the synthetic `BWS_ACCESS_TOKEN` value, credential-like values, or authorization headers; then make sanitization unconfirmable after worker execution. | Sanitize every AI-visible and persistent output, including the synthetic `BWS_ACCESS_TOKEN` value. If sanitization cannot be confirmed, discard the affected content, fail closed with `redaction_failure`, revoke or delete the credential, update the pending record, and complete cleanup before returning the error. | No secret-bearing value, including the synthetic `BWS_ACCESS_TOKEN` value, remains in AI-visible output, worker stdout/stderr, audit data, pending records, process environments, or unrelated child processes. After `redaction_failure`, the same token is not retained and cleanup completes successfully. The terminal event records only `failure_category=redaction_failure` plus secret-free metadata; a cleanup failure records `credential_residue` with identifier and expiry and emits the cleanup audit event. |
...
| EVAL-14 | Generate audit events for pre-issuance rejection, issuance, execution, failure, revocation, and recovery. | Write append-only records to the broker-owned location and retain them for at least 90 days. | Rejection events use `request_rejected`, `issuance_status=not_issued`, and `revocation_status=not_issued`, and include `work_id`, `intent_state`, sanitized request, and authorization context; all events contain the required schema fields, exclude secrets and reversible encodings, and cannot be altered by the AI worker. Retention evidence is deterministic: either the retention policy is inspected and enforces at least 90 days, or a deterministic clock verifies that audit events remain readable after 90 days; implementations that delete records before 90 days fail. |
