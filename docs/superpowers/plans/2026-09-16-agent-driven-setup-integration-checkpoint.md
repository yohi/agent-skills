...
| MCP P1 chain | Stdio fixture supports initialize, tools/list, and a safe `read_only` tool call; the representative item declares `tool`, `arguments`, and `safety: read_only` | Run target probe once per verification item ID in normal mode, then run dry-run admission | Each supported item yields evidence in normal mode; `temporary_fixture` is not a P1 operation; dry-run marks unsupported temporary, mutating, and unknown operations `not_executed`; target E2E is derived from items and no phase-only selection occurs. |
| Unsupported temporary fixture | MCP representative call declares an external `temporary_fixture` surface with `snapshot: required` and `cleanup_required: true`, with and without a Contract-defined handoff | Run normal target probe and dry-run admission | Normal verification starts neither the representative operation nor cleanup and returns `not_verified` / `safety_blocked`, emitting only the defined handoff; dry-run returns `not_executed`; no external mutation occurs. |
...
| Generic P1 boundary | Policy v1 exact fixtures `node --version`, `git status --porcelain`, `npm install`, `unknown-command --check`, absolute-path/wrapper variants, plus Plugin/Hook/custom fixture | Run probes in normal mode and safety admission in dry-run | Only the two named known read-only commands run; `npm install`, unknown variants, and mismatches are `not_verified` / `safety_blocked` in normal mode and `not_executed` in dry-run; unsupported types return only defined handoff. |
...
| 10 | MCP includes initialize, discovery, and possible tool call | Stdio MCP fixture exercises the three P1 items; the supported representative call uses Contract-defined `tool`, `arguments`, and `safety: read_only`, while `temporary_fixture` is an explicitly unsupported P1 mode. |
