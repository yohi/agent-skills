...
- Command and Skill adapter safety declarations are claims, not execution permission; `run-target-probes.sh` applies the fixed effective-safety policy and rejects mutating, unknown, or mismatched argv before process start. MCP `initialize` / `tool_discovery` remain fixed read-only protocol operations, while MCP representative-call safety follows its Contract-defined mode and requires an external `snapshot: required` / `cleanup_required: true` surface for `temporary_fixture`.
- Normal verification permits read-only probes and only Contract-defined MCP `temporary_fixture`; dry-run permits known read-only probes and marks temporary, mutating, and unknown operations `not_executed` without starting them.
...
**Produces:** additive dry-run object with `dry_run.decision: execute|not_executed`, external temporary cache, before/after snapshot, explicit target-probe safety decisions, and exit codes 1 through 3.

**Interfaces:** normal `category` remains `safe|review`; dry-run never uses `safe` or `review` as its decision; CLI parsing and simple-path selection do not require PyYAML; the legacy path is selected when there is no trigger or Contract indicator; the enhanced path preflights PyYAML before canonical Contract discovery or any target write; dry-run classifies effective `read_only` as `execute` and `temporary_fixture`, `mutating`, and `unknown` as `not_executed` before process start; malformed grammar/Contract is exit 2 and missing enhanced-path PyYAML is exit 3 with no install attempt.
...
- [ ] Add fixtures asserting `node --version` and `git status --porcelain` are `dry_run.decision == "execute"`, while `npm install`, a Contract-declared MCP `temporary_fixture`, and `unknown-command --check` are `"not_executed"` without process start.
...
- [ ] Parse CLI and perform the read-only, PyYAML-free path-selection gate first. Keep the no-trigger/no-Contract simple path on the existing workflow without PyYAML; only after enhanced path selection preflight PyYAML, discover/read the Contract, create target-external temporary storage, and take the before snapshot before any target write. Apply dry-run safety admission before invoking any target probe.
- [ ] In dry-run, analyze through the external temporary cache; never create or update the target `.agent-setup` cache.
- [ ] Classify only known read-only commands and adapters as `execute`; suppress temporary fixtures, mutating, and unknown commands/adapters as `not_executed`; emit invariant violation and exit 1 on an unexpected local mutation, snapshot failure, or temporary-storage cleanup failure.
...
**Consumes:** `run-target-probes.sh --contract <path> --target <target-id> --item <verification-item-id> --evidence-dir <external-temp-dir>` and Contract `probe` definitions, including explicit MCP arguments/safety, required command/adapter safety declarations, and Skill adapter definitions.

**Produces:** one JSON object `{target_id, item_id, status, reason, evidence, error_category}` per invocation.

**Interfaces:** `--item` selects one target-local verification item; the executor derives phase from that item and never selects by phase alone. Executor exclusively owns process lifecycle, timeout, temporary fixture cleanup, target operation execution, and final safety enforcement immediately before process start. `command.safety` and `agent_action.adapter.safety` are required declarations with `read_only|mutating|unknown`; the executor's fixed policy classifies known-mutating signatures first, then a known-read-only allowlist, and defaults to `unknown`. A declaration is not execution permission, and a declaration/effective-class mismatch is safety-blocked. Normal verification executes only effective `read_only` probes plus Contract-defined MCP `representative_tool_call` with `safety: temporary_fixture`; that temporary fixture requires an external `snapshot: required` / `cleanup_required: true` surface and an executor-applicable cleanup procedure. Dry-run executes only effective `read_only` probes and marks temporary, mutating, and unknown operations `not_executed` before process start. MCP representative calls use Contract-provided `tool`, JSON-object `arguments`, `safety`, and required `mutation_surface_id` for temporary fixtures; Skill `agent_action` uses Contract-provided public adapter `argv` and prompt stdin only after the safety gate. Adapter exit 0 plus observable response is `verified`, unavailable mechanism is `capability_unavailable`, non-zero exit is `runtime_failure`, and a normal safety rejection, unsupported mutation surface, or temporary cleanup/snapshot failure is `not_verified` / `safety_blocked`; status is `verified|not_verified|not_applicable`; error category is `null|capability_unavailable|runtime_failure|audit_blocked|safety_blocked`.

`cleanup_required: true` does not permit a Contract-provided cleanup command. The executor uses a predefined cleanup path for the mutation-surface kind and transport; if no applicable path exists, it blocks before operation start as `unsupported mutation surface` / `safety_blocked`.

**RED:**
- [ ] Add an MCP stdio fixture that answers initialize and tools/list, a fixture whose initialize returns an error, a safe representative tool fixture with explicit `tool`, `arguments`, and `safety` plus an external `snapshot: required` / `cleanup_required: true` temporary mutation-surface reference when needed, a normal-verification temporary-fixture cleanup-failure fixture, a Skill discovery/activation fixture with an explicit public adapter and separate unsafe/unknown-safety adapter fixtures, known-read-only/known-mutating/unknown command fixtures, and a Plugin fixture with only a handoff.
- [ ] Assert that normal known-mutating/unknown commands and unsafe/unknown/mismatched adapters do not start a process and return `status: not_verified` with `error_category: safety_blocked`; assert that the same operations in dry-run return `dry_run.decision: not_executed`.
- [ ] Assert that a normal Contract-defined MCP `temporary_fixture` is admitted only with an external `snapshot: required` / `cleanup_required: true` surface and an executor-applicable cleanup path, that an unsupported surface is blocked before operation start, that cleanup failure returns `not_verified` / `safety_blocked`, and that dry-run never starts the temporary operation.
- [ ] Run the suite and confirm failure because the executor does not exist and no probe JSON can be parsed.

**GREEN:**
- [ ] Implement P1 MCP stdio JSON-RPC `initialize`, `tool_discovery`, and the Contract-selected safe representative tool call using its explicit `tool`, `arguments`, `safety`, and any required `mutation_surface_id`, with timeout and termination. Permit `temporary_fixture` only in normal verification when the external `snapshot: required` / `cleanup_required: true` surface is Contract-defined and the executor has an applicable cleanup procedure; never invoke it in dry-run.
...
- [ ] On normal temporary-fixture cleanup or snapshot verification failure return `not_verified` / `safety_blocked`; on target operation error return `not_verified` / `runtime_failure` without changing capability availability. In dry-run, temporary, mutating, and unknown operations are `not_executed`, while an unexpected local mutation or temporary-storage cleanup failure is `dry_run_invariant_violation` with exit 1.
...
- [ ] Document the fixed safety order: Contract declaration, executor effective classification, process-start enforcement, and deterministic `safety_blocked` / `not_executed` result mapping.
