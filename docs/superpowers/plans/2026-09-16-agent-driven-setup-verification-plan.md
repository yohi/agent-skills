...
- Design の Probe Safety Policy v1 が command / Skill adapter / process runtime の canonical registry、argv normalization、exact matching、fallback を所有する。`run-target-probes.sh` が分類実装の単一 owner であり、通常 verification は command probe、supported runtime、または将来明示的に supported となる adapter の process start 直前に最終 gate を適用する。P1 Skill adapter は support boundary により process start へ進めず、dry-run は `--classify-only` で同じ分類結果を取得する。MCP `initialize` / `tool_discovery` は固定 read-only protocol operation とする。
- Normal verification は実効 safety が `read_only` の command / supported MCP runtime と MCP `safety: read_only` representative call だけを実行する。Skill `agent_action` discovery / activation は P1 では自動実行せず、Contract-defined handoff 境界とする。Skill adapter は classifier が `read_only` を返しても通常 verification では `not_verified` / `safety_blocked`、dry-run では `not_executed` とし、P1 は temporary fixture の runtime process、外部 resource の operation、adapter process、cleanup を自動実行しない。
...
**Interfaces:** capability status is exactly `available`, `unavailable`, or `unknown`; Spec 2 resolves each Contract capability ID against `setup-capability-matrix.md`, with a missing definition reported as `unknown` and its dependent item as `not_verified`; P1 supports MCP stdio only when `runtime.command` and `runtime.safety` pass the Policy v1 read-only gate, plus read-only CLI/Service commands whose effective safety is `read_only`; Skill `agent_action` discovery / activation is a Contract-defined handoff boundary and is not automatically executed in P1; MCP `temporary_fixture` is accepted by the Contract shape but unsupported for automatic P1 execution; Plugin, Hook, and `other/custom` return Contract-defined handoff without invented probes. Safety-blocked items are `not_verified` and use `error_category: safety_blocked`.
...
**Consumes:** `verify-setup.sh [--dry-run] [--contract <repo-relative-path>] [--report <target-external-markdown-path>] <repo-path>`, the PyYAML-free legacy/enhanced path-selection gate, Setup Contract discovery, Probe Safety Policy v1 and its `run-target-probes.sh --classify-only` interface, process runtime safety declarations, and mutation-surface semantics.
...
**Interfaces:** normal `category` remains `safe|review`; dry-run never uses `safe` or `review` as its decision; CLI parsing and simple-path selection do not require PyYAML; the legacy path is selected when there is no trigger or Contract indicator; the enhanced path preflights PyYAML before canonical Contract discovery or any target write; dry-run invokes `run-target-probes.sh --classify-only` for each command/adapter/runtime argv, classifies effective `read_only` as `execute` for command probes, classifies Skill `agent_action` adapters for evidence but applies the P1 handoff boundary as `not_executed` without starting a process, records MCP runtime startup as `not_executed` without starting a process, and classifies `temporary_fixture`, `mutating`, `unknown`, or safety mismatch as `not_executed` before target process start; malformed grammar/Contract is exit 2 and missing enhanced-path PyYAML is exit 3 with no install attempt.
...
- [ ] Add fixtures asserting the Policy v1 entries `node --version`, `git status --porcelain`, and the exact MCP runtime `agent-setup-mcp-stdio-readonly` are classified as `read_only`, while `npm install`, a Contract-declared MCP `temporary_fixture`, and `unknown-command --check` are `"not_executed"` without target process start; the MCP runtime fixture is classified but not started in dry-run.
...
- [ ] Run the suite and confirm failure because the current script writes `.agent-setup` before classification, has no new grammar, and does not apply the runtime-start admission rule.

**GREEN:**
- [ ] Parse all options before one required repository path and preserve default stdout JSON.
- [ ] Parse CLI and perform the read-only, PyYAML-free path-selection gate first. Keep the no-trigger/no-Contract simple path on the existing workflow without PyYAML; only after enhanced path selection preflight PyYAML, discover/read the Contract, create target-external temporary storage, and take the before snapshot before any target write. Ask `run-target-probes.sh --classify-only` for every command/adapter/runtime classification; apply the P1 Skill handoff boundary before any adapter admission, apply the shared classification before invoking any supported target probe, and never start an MCP runtime in dry-run.
- [ ] In dry-run, analyze through the external temporary cache; never create or update the target `.agent-setup` cache.
- [ ] Use only the Policy v1 classifier result: effective known read-only command probes with matching declarations are `execute`; P1 Skill adapters may be classified but remain `not_executed` without process start; the known read-only MCP runtime fixture is classification-only and `not_executed` in dry-run; temporary fixtures, mutating, unknown, and mismatched commands/adapters/runtimes are `not_executed` without target process start; emit invariant violation and exit 1 on an unexpected local mutation, snapshot failure, or temporary-storage cleanup failure.
...
**RED:**
- [ ] Add fixtures for an undefined setup capability, unavailable `mcp_runtime_probe`, Skill discovery / activation requiring the P1 handoff boundary, available MCP runtime with initialize failure, and an item without `handoff_id`.
- [ ] Run the suite and confirm failure because assessment and target result are not separate fields.

**GREEN:**
- [ ] Implement concrete, non-mutating environment/observability checks for each P1 capability, while keeping capability availability separate from the Skill handoff boundary and MCP runtime-start safety gate.
...
**Consumes:** `run-target-probes.sh --contract <path> --target <target-id> --item <verification-item-id> --evidence-dir <external-temp-dir>`, its side-effect-free `--classify-only` interface, Design の Probe Safety Policy v1, and Contract `probe` definitions, including explicit MCP arguments/safety, required command/adapter safety declarations, process `runtime.command` / `runtime.safety`, and Skill adapter definitions.

**Produces:** one fixed JSON object `{target_id, item_id, status, reason, evidence, error_category}` per normal probe invocation; `--classify-only` produces one classification object `{effective_safety, declaration_matches}` per classification invocation.

**Interfaces:**
- `--item` selects one target-local verification item; the executor derives phase from that item and never selects by phase alone.
- `--classify-only` reads one JSON object from stdin, `{"argv":[...],"declared_safety":"read_only|mutating|unknown"}`, and emits one JSON object, `{"effective_safety":"read_only|mutating|unknown","declaration_matches":true|false}`, without reading or writing a target or starting a target operation.
- The executor exclusively owns the registry, classifier, process lifecycle, timeout, target operation execution, and final safety enforcement immediately before process start. The Policy v1 registry is exactly `["node", "--version"]`, `["git", "status", "--porcelain"]`, and `["agent-setup-mcp-stdio-readonly"]` as `read_only`, plus `["npm", "install"]` as `mutating`; matching is full argv exact match with no path normalization, wrapper unwrapping, or trailing arguments, and every other argv is `unknown`.
- A declaration is not execution permission, and a declaration/effective-class mismatch is safety-blocked. Normal verification executes only effective `read_only` command probes and the MCP stdio chain whose `runtime.command` and `runtime.safety` pass the same gate. The concrete P1 runtime fixture is `runtime.command: ["agent-setup-mcp-stdio-readonly"]` with `runtime.safety: read_only`.
- Dry-run invokes the same `--classify-only` classifier for every command, adapter, and runtime argv. Effective `read_only` command probes may be `execute`; P1 Skill `agent_action` adapters may be classified as `read_only` but remain `not_executed` without starting a process; MCP runtime startup is always `not_executed` in dry-run, even when classified `read_only`, and no MCP process or protocol request starts.
- Contract-defined MCP `temporary_fixture` is schema-valid but unsupported for automatic P1 execution: normal verification returns `not_verified` / `safety_blocked` before runtime process, operation, or cleanup, while dry-run returns `not_executed`. Skill `agent_action` discovery / activation is also a P1 handoff boundary and does not start an adapter process; in dry-run, a Contract-defined handoff definition is reference-only and no handoff execution record is created.
- MCP representative calls use Contract-provided `tool`, JSON-object `arguments`, and `safety`; Skill handoff records retain Contract-provided adapter data without inventing a public Agent seam. Adapter exit 0 plus observable response is `verified` only for a future explicitly supported seam; unavailable mechanism is `capability_unavailable`, non-zero exit is `runtime_failure`, and a normal safety rejection or unsupported probe mode is `not_verified` / `safety_blocked`; status is `verified|not_verified|not_applicable`; error category is `null|capability_unavailable|runtime_failure|audit_blocked|safety_blocked`.

`cleanup_required: true` does not permit a Contract-provided cleanup command. P1 does not define or invoke a transport, snapshot method, or cleanup path for `temporary_fixture`; the executor blocks before runtime process, operation, or cleanup start as `unsupported probe mode` / `safety_blocked`, and emits only a Contract-defined handoff when one exists.

**RED:**
- [ ] Add an MCP stdio fixture that answers initialize and tools/list, a fixture whose initialize returns an error, and a safe `read_only` representative tool fixture with explicit `tool`, `arguments`, and `safety`; use the exact safe runtime `runtime.command: ["agent-setup-mcp-stdio-readonly"]` with `runtime.safety: read_only`; add an unsupported `temporary_fixture` fixture with an external `snapshot: required` / `cleanup_required: true` mutation-surface reference and an optional Contract-defined handoff; add a Skill discovery/activation fixture with a Contract-defined handoff and an adapter that must not start; add the named Policy v1 command fixtures `node --version`, `git status --porcelain`, `npm install`, and `unknown-command --check`, plus absolute-path and wrapper variants that must remain `unknown`; add safe, mutating, unknown, and mismatch runtime variants; add a Plugin fixture with only a handoff.
- [ ] Assert that normal known-mutating/unknown commands and runtimes do not start a target process and return `status: not_verified` with `error_category: safety_blocked`; assert that every Skill discovery/activation adapter, including a declared-safe adapter, is handed off without starting a process in normal mode and is `dry_run.decision: not_executed` without starting a process in dry-run; for each same-argv normal/dry-run pair, assert identical `effective_safety` and `declaration_matches`, then assert normal rejection versus dry-run `dry_run.decision: not_executed`.
- [ ] Assert that a normal Contract-defined MCP `temporary_fixture` is blocked before runtime process, representative operation, and cleanup starts with `status: not_verified` / `error_category: safety_blocked`, that its defined handoff is the only handoff emitted, and that dry-run returns `dry_run.decision: not_executed` without starting the temporary runtime.
- [ ] Run the suite and confirm failure because the executor does not exist and no probe JSON can be parsed.

**GREEN:**
- [ ] Implement P1 MCP stdio JSON-RPC `initialize`, `tool_discovery`, and the Contract-selected `safety: read_only` representative tool call using its explicit `tool`, `arguments`, and safety, with timeout and termination. Before server process start, classify `runtime.command` with `runtime.safety` through the shared Policy v1 gate; allow only the exact safe fixture `agent-setup-mcp-stdio-readonly` in normal verification, reject mutating/unknown/mismatched runtime commands before process start, and never start the MCP process in dry-run.
- [ ] Keep Skill `agent_action` discovery/activation as a P1 handoff boundary. Do not execute the Contract-provided adapter, even when its declaration would match a future safe registry entry; in normal verification return `not_verified` / `safety_blocked` and emit only its Contract-defined handoff when one exists, while in dry-run return `dry_run.decision: not_executed` without an adapter process and include only the handoff definition as reference, not an execution record.
...
- [ ] On normal unsupported `temporary_fixture` mode or unsafe/unknown/mismatched runtime command return `not_verified` / `safety_blocked` before any runtime process, external operation, or cleanup; on target operation error return `not_verified` / `runtime_failure` without changing capability availability. In dry-run, MCP runtime startup, temporary, mutating, mismatched, and unknown operations are `not_executed`, while an unexpected local mutation or temporary-storage cleanup failure is `dry_run_invariant_violation` with exit 1.
...
**Interfaces:** Contract defines handoff; report records execution; Agent does not construct an undefined handoff; `blocked_by` skips dependent probes; Skill `agent_action` discovery / activation remains a P1 handoff boundary and starts no adapter process; MCP runtime startup is admitted only after the Policy v1 gate; a normal `safety_blocked` item emits only its Contract-defined handoff as pending/needs_review, while a dry-run Skill adapter is `not_executed`, includes only the handoff definition as reference when one is defined, and does not create a handoff execution record or target runtime failure.
...
- [ ] Document the fixed safety order: Contract declaration, Policy v1 classification through the executor (normal final gate or dry-run `--classify-only`), P1 Skill support-boundary handoff before adapter execution admission, MCP runtime process-start enforcement, and deterministic `safety_blocked` / `not_executed` result mapping.
